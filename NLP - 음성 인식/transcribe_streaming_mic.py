...
credential_path = "/home/piai/Downloads/nlp-first-dc313e65108a.json"
...
def listen_print_loop(responses):
    ...
    for response in responses:
        ...
        if not result.is_final:
            ...
            print("원래 문장 : " + transcript)
            # sen_split = kkma.morphs(transcript)
            sen_split = komoran.morphs(transcript)
            ...
            pred = model(w2v_tensor_ch)
            result = torch.max(pred, 1)

            result_idx = result.indices.item()
            result_val = result.values.item()
            share      = result_idx

            cli.set('share_mic', share)
            print("위험판단(0 : 안전, 1 : 위험) : {}  확률 : {}".format(result_idx,result_val))
